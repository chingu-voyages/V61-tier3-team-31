  ...
  return (
    <div className="flex items-center gap-3 p-3">
      ...
      <div className="min-w-0">
        <div className="font-outfit text-xl font-bold text-slate-800 dark:text-white leading-none">
          {value}
        </div>
...
const deadlines = [
  {
    ...
    date: "May 2, 2026",
    badge: "3 days",
    badgeColor: "bg-rose-50 dark:bg-rose-500/10 text-rose-500",
  },
  {
    title: "Confirm teams",
    date: "May 9, 2026",
...
const attentionItems: Array<{
  ...
  {
    icon: <AlertTriangle />,
    color: { bg: "bg-orange-50 dark:bg-orange-500/10", text: "text-orange-500" },
...
const activityItems = [
  {
    ...
    icon: Check,
    iconColor: { bg: "bg-[#77CF97]/20", text: "text-[#77CF97]" },
  ...
  },
  {
    ...
    icon: UsersRound,
    iconColor: { bg: "bg-purple-100 dark:bg-purple-500/20", text: "text-purple-600" },
  ...
  },
  {
    ...
    icon: FileSignature,
    iconColor: { bg: "bg-blue-100 dark:bg-blue-500/20", text: "text-blue-500" },
  ...
  },
  {
    ...
    icon: XCircle,
    iconColor: { bg: "bg-rose-100 dark:bg-rose-500/20", text: "text-rose-500" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-[#1a1b24] rounded-[24px] border border-slate-100 dark:border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.02)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] p-8 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-3">
            <MetricBlock
              icon={<FileText />}
              color={{ bg: "bg-[#77CF97]/10", text: "text-[#77CF97]" }}
              value="312"
              label="Applications"
        ...
        <div className="lg:col-span-4 bg-white dark:bg-[#1a1b24] rounded-[24px] border border-slate-100 dark:border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.02)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] p-6 flex flex-col">
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              ...
              className="text-xs font-medium text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors cursor-pointer"
            >
              View all
            </button>
          </div>
      ...
      <div>
        <div className="flex justify-between items-end mb-4 px-1">
          ...
          <button
            ...
            className="text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors cursor-pointer"
          >
            View full pipeline
          </button>
        </div>
          ...
          {pipelineSteps.map((p, i) => (
            ...
            >
              <div className="flex gap-3.5 items-start mb-6">
                ...
                <div className="pt-1">
                  <div className="font-semibold text-slate-800 dark:text-white text-sm leading-tight mb-1">
                    {p.title}
                  </div>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="text-[10px] font-medium text-slate-500 dark:text-slate-400 whitespace-nowrap uppercase tracking-wider">
                  {p.total}
                </div>
              </div>
              <div className="space-y-3 mb-8 flex-1">
                {p.stats.map((s, j) => (
                  <div key={j} className="flex justify-between items-center text-xs">
                    ...
                    <span
                      className={`font-semibold ${s.color || "text-slate-900 dark:text-white"}`}
                    >
                      {s.value}
                    </span>
                  </div>
          ...
          <div className="space-y-1">
            {attentionItems.map((item, i) => (
              <div
                ...
                className="flex items-center gap-4 py-3 border-b border-slate-50 dark:border-white/5 last:border-0 last:pb-0 cursor-pointer group"
              >
                <div
                  className={`w-10 h-10 rounded-xl border border-rose-100/50 dark:border-white/10 shrink-0 shadow-sm flex items-center justify-center [&>svg]:w-5 [&>svg]:h-5 ${item.color.bg} ${item.color.text}`}
                >
                  {item.icon}
                </div>
                <div className="flex-1 pt-0.5">
                  <div className="text-sm font-medium text-slate-800 dark:text-white group-hover:text-rose-500 transition-colors mb-0.5">
                    {item.title}
                  </div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">{item.desc}</div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-xs font-semibold text-rose-500 bg-rose-50 dark:bg-rose-500/10 px-2 py-0.5 rounded-md">
                    {item.value}
                  </div>
          ...
          <div className="space-y-1">
            {activityItems.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex gap-4 items-start py-3 border-b border-slate-50 dark:border-white/5 last:border-0 last:pb-0 cursor-pointer group"
                >
                  <div className="relative shrink-0 mt-0.5">
                    ...
                    />
                    <div
                      className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-white dark:border-[#1a1b24] flex items-center justify-center ${item.iconColor.bg}`}
                    >
...
/** Fortschrittsring fuer das Onboarding */
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-semibold border border-blue-100 dark:border-blue-500/20">
          <CheckCircle className="w-3 h-3" /> Onboarding: {t.onboarding.completed}/
          {t.onboarding.total}
        </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 lg:grid-rows-[auto_1fr] gap-4 auto-rows-min">
        ...
        <div className="lg:col-span-3 bg-white dark:bg-[#1a1b24] p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-5">
            Onboarding Progress
          </h3>
            ...
            <div className="flex-1 space-y-3">
              <div>
                <div className="text-sm font-bold text-slate-900 dark:text-white">
                  {t.onboarding.completed} of {t.onboarding.total} steps completed
                </div>
        ...
        <div className="lg:col-span-1 lg:row-span-2 bg-white dark:bg-[#1a1b24] p-5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#77CF97]/10 text-[#77CF97] border border-[#77CF97]/20 capitalize shrink-0">
                  {t.team.status}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-1 mb-4">
            {t.team.members.map((m, i) => (
              <img
                key={i}
                src={`https://i.pravatar.cc/100?img=${m.avatar}`}
                className="w-9 h-9 rounded-full border-2 border-white dark:border-[#1a1b24] bg-slate-100 dark:bg-white/10"
                alt={m.name}
                title={`${m.name} (${m.role})`}
              />
            ))}
            <span className="w-9 h-9 rounded-full border-2 border-white dark:border-[#1a1b24] bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-slate-400 text-[10px] font-medium flex items-center justify-center">
              {t.team.memberExtra}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 mb-3 flex-wrap">
            {t.team.roles.map((role, i) => (
              <span
                key={i}
                className="px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium"
              >
                {role}
              </span>
            ))}
        ...
        <div className="bg-white dark:bg-[#1a1b24] p-5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="text-[10px] font-medium text-[#77CF97] hover:underline cursor-pointer"
            >
              View All
            </button>
          </div>
          <div className="space-y-2.5 flex-1">
            {t.upcomingDates.slice(0, 3).map((d, i) => (
              <div
                key={i}
                className="flex items-center gap-2.5 p-2 rounded-xl border border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors cursor-pointer"
              >
            ...
            {t.announcements.map((item, i) => (
              <div key={i} className="flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-medium text-slate-800 dark:text-white leading-snug">
                    {item.text}
                  </div>
        ...
        <div className="flex items-center gap-2 mb-8">
          {["Submitted", "Under Review", "Decision", "Onboarding"].map((step, i) => (
            <div key={i} className="flex items-center gap-2 flex-1">
              ...
              <span
                className={`text-xs font-medium ${i === 0 ? "text-[#77CF97]" : "text-slate-400 dark:text-slate-500"}`}
              >
                {step}
              </span>
              {i < 3 && (
                <div
                  className={`flex-1 h-[2px] ${i === 0 ? "bg-[#77CF97]" : "bg-slate-200 dark:bg-white/10"}`}
                ></div>
              )}
            </div>
          ))}
