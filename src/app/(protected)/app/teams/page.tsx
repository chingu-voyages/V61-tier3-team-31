...
const adminTeams = [
  {
    ...
    description:
      "Building a decentralized marketplace for digital assets with cross-chain compatibility and zero gas fees.",
    progress: 75,
  ...
  },
  {
    ...
    description:
      "An AI-powered VS Code extension for real-time refactoring and style guide enforcement for Next.js.",
    progress: 30,
  ...
  },
  {
    ...
    statusIcon: CheckCircle,
    description:
      "Mental health journal app utilizing sentiment analysis to track mood trends and recommend mindfulness exercises.",
    progress: 90,
  ...
  },
  {
    ...
    statusIcon: CheckCircle,
    description:
      "Micro-budgeting tool aiming to help college students automatically round up savings across multiple bank accounts.",
    progress: 55,
  ...
  },
  {
    ...
    statusIcon: Clock,
    description:
      "Visualizing open-source marine data to track coral bleaching events and climate impact.",
    progress: 10,
  ...
  },
  {
    ...
    statusIcon: CheckCircle,
    description:
      "Browser-based multiplayer trivia game using web sockets and real-time question generation via LLM.",
    progress: 100,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-px">
        <div className="flex gap-2">
          <button className="px-4 py-2 border-b-2 border-slate-900 dark:border-white text-slate-900 dark:text-white font-medium text-sm cursor-pointer shadow-sm">
            All Teams (18)
          </button>
          <button className="px-4 py-2 border-b-2 border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-medium text-sm cursor-pointer">
            Active (15)
          </button>
          <button className="px-4 py-2 border-b-2 border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-medium text-sm cursor-pointer">
            At Risk (2)
        ...
        <div className="relative mb-2">
          ...
          <input
            ...
            className="w-64 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-slate-300 dark:focus:border-white/20 transition-colors shadow-sm"
          />
        </div>
      </div>
        ...
        {adminTeams.map((team, i) => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-[#1a1b24] rounded-[24px] border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md dark:hover:shadow-[0_4px_20px_rgba(0,0,0,0.3)] hover:border-slate-300 dark:hover:border-white/20 transition-all p-6 flex flex-col cursor-pointer"
            >
              ...
              <div className="mt-auto">
                ...
                <div className="flex items-center justify-between border-t border-slate-100 dark:border-white/10 pt-4">
                  ...
                  <button
                    ...
                    className="text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors cursor-pointer"
                  >
                    View Details
                  </button>
                </div>
  ...
  members: [
    {
      ...
      role: "Frontend",
      timezone: "UTC-7",
      availability: "Mon-Thu, 18:00-22:00",
      github: "oliviachen",
      isYou: true,
    },
    {
    ...
    },
    {
      ...
      role: "Backend",
      timezone: "UTC+0",
      availability: "Mon-Fri, 18:00 - 22:00",
      github: "sophiataylor",
    },
    {
      name: "James Liu",
        ...
        <div className="min-w-[580px]">
                ...
                <div key={i} className="flex items-center" style={{ height: ROW_H }}>
                  ...
                  <div className="flex-1 relative" style={{ height: ROW_H }}>
                    {ticks.map((h) => (
                      <div
                        key={h}
                        className="absolute top-0 bottom-0 border-l border-slate-100 dark:border-white/5"
                        style={{ left: pct(h) }}
                      />
                    ))}
                    <div
          ...
          <div className="flex items-center gap-2 mt-4 ml-[160px]">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">
              Best shared overlap: {String(overlapStart).padStart(2, "0")}:00 –{" "}
              {String(overlapEnd).padStart(2, "0")}:30 UTC
            </span>
          </div>
        </div>
      </div>
              ...
              <div className="flex items-start gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-2.5">
                    You are viewing this team as a participant. Only admins can make changes.
                  </p>
                </div>
              </div>
                ...
                <div className="flex -space-x-2">
                  {t.members.slice(0, 5).map((m, i) => (
                    <img
                      ...
                      className="w-9 h-9 rounded-full border-2 border-white dark:border-[#1a1b24]"
                      alt={m.name}
                      title={m.name}
                    />
                  ))}
          ...
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {t.members.map((m, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-[#1a1b24] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm p-5 hover:shadow-md dark:hover:shadow-lg transition-shadow"
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {m.isYou && (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400">
                            You
                          </span>
                        )}
                      </div>
                  ...
                  <Link
                    href="/profile"
                    className="block w-full text-center py-2 border border-slate-200 dark:border-white/10 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
                  >
                    View Profile →
                  </Link>
                </div>
              ...
              <div className="relative w-12 h-12">
                <svg className="w-12 h-12 -rotate-90" viewBox="0 0 48 48">
                  ...
                  <circle
                    ...
                    strokeWidth="3"
                    strokeLinecap="round"
                    strokeDasharray={`${0.8 * 2 * Math.PI * 20} ${2 * Math.PI * 20}`}
                  />
                </svg>
                <span className="absolute inset-0 flex items-center justify-center text-[11px] font-bold text-slate-800 dark:text-white">
                  80%
                </span>
              </div>
            </div>
              ...
              {t.roleCoverage.map((rc, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <div
                      className={`w-9 h-9 rounded-xl ${s.bg} ${s.text} flex items-center justify-center shrink-0`}
                    >
                      {s.icon}
                    </div>
                    <span className="text-sm font-semibold text-slate-900 dark:text-white w-[72px] shrink-0">
                      {rc.role}
                    </span>
                    <span
                      className={`text-sm flex-1 ${rc.filled ? "text-slate-600 dark:text-slate-300" : "text-slate-400 dark:text-slate-500 italic"}`}
                    >
                      {rc.name ?? "Vacant"}
                    </span>
                    {rc.filled ? (
